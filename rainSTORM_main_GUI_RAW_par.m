...
% 2013/02/20 Modified by Alex Knight for parallel processing of .RAW data
% 
% Method
...
function yy=rainSTORM_main_GUI_RAW(filename,initX0,initSig,rad,tol,Thresh,maxIts,flagSB,alg,flagSum,pixelWidth)
...
% The file is assumed to be a RAW file
fprintf( 'Parallel RAW file processing\n ' );

[ImSzX,ImSzY,numberOfFiles] = rainSTORM_xmlread(filename);
frameSize = [ImSzX,ImSzY,];
...
SupResPosits = []; % To hold [row,col] of fits
SupResParams = []; % And [Str,Res,Int,SigmaX,SigmaY,Precision,Frame]

SupResNumber = 0; % Current write row in SupResPosits, etc.
...
parfor lpIm = 1:numberOfFiles; 
 ...
 frameOffset = (lpIm - 1) * ImSzX * ImSzY * 2;
 myFits = [];
...
[SupResIm] = rainSTORM_recon(SupResPosits,SupResParams,prevSF,frameSize); 
...
if(flagSum) 
    ...
    for lpImSum = 1:(numberOfFiles);
        ...
        myFrameIn=myFrameIn'; % Image orientation
        myFrame32 = uint32(myFrameIn);
        sumFrame = sumFrame + myFrame32;
...
assignin('base','myFrame',zeros(ImSzX,ImSzY)); % Need size info. Improve..
...
assignin('base','sizeOfCCDFrame',frameSize ); % Size of CCD image
